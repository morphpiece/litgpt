...
class FineWeb(DataModule):
    ...
    config: Literal['10B', '100B', '350B'] = '10BT'

    def __post_init__(self):
        # Could be a remote path (s3://) or a local path
        self.fineweb_train = str(self.data_path).rstrip(
            "/") + "/fineweb/" + self.config + "/train"
        self.fineweb_dev = str(self.data_path).rstrip(
            "/") + "/fineweb/" + self.config + "/dev"
    ...
    def prepare_data(self) -> None:
        for path in (self.fineweb_train, self.fineweb_dev):
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        train_dataloader = StreamingDataLoader(
            train_datasets, batch_size=self.batch_size, pin_memory=True, num_workers=self.num_workers, drop_last=True
        )
